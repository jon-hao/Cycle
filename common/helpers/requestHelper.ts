...
import { TFetchDataOption, TQuery, TRequest } from "../types/httpType";

export const parseRequest = (ctx: Context): TRequest => {
  const query = ctx.query as TQuery || {};
...
export const fetchData = async (uri: string, fetchDataOption: TFetchDataOption): Promise<any> => {
  try {
    let url = uri;
